...
function useLenis() {
  useEffect(() => {
    document.documentElement.style.scrollBehavior = "smooth"; /* Alle Anchor-Links scrollen sanft */
    return () => {
      document.documentElement.style.scrollBehavior = ""; /* Cleanup: Standardverhalten wiederherstellen */
    };
...
function ScrollProgress() {
  ...
  return (
    <motion.div
      style={{ scaleX, transformOrigin: "left" }} /* scaleX: 0 = kein Balken, 1 = voller Balken */
  ...
  return (
    <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden bg-[#050508]">
      ...
      <motion.div
        ...
        style={{
          background: "radial-gradient(ellipse, #ec4899 0%, transparent 70%)", /* Pink nach transparent */
        ...
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }} /* 18s-Schleife */
      />
      ...
      <motion.div
        ...
        style={{
          background: "radial-gradient(ellipse, #a855f7 0%, transparent 70%)", /* Lila */
          right: "-8%",
          top: "-10%",
        }}
        animate={{
        ...
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }} /* 22s — anderer Rhythmus */
      />
      ...
      <motion.div
        ...
        style={{
          background: "radial-gradient(ellipse, #6366f1 0%, transparent 70%)", /* Indigo */
          left: "15%",
          bottom: "-5%",
        }}
        animate={{
        ...
        transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }} /* 15s — schnellster */
      />
      ...
      <div
        ...
        style={{
          ...
          backgroundSize: "36px 36px", /* Gittergröße: 36px horizontal + vertikal */
        }}
      />
...
function FloatingShapes() {
  return (
    <div className="fixed inset-0 z-[1] pointer-events-none overflow-hidden">
      ...
      <div className="absolute top-[8%] right-[6%]"> {/* Ankerpunkt für beide Ringe */}
        ...
        <div
          ...
          style={{
            width: 160,
            height: 160,
            border: "1px dashed rgba(168,85,247,.12)", /* Gestrichelt, lila, sehr dezent */
      ...
      <div
        ...
        style={{
          width: 200,
          height: 200,
          border: "1px dashed rgba(99,102,241,.08)", /* Indigo, fast unsichtbar */
          borderRadius: "50%",
        }}
      />
      ...
      <motion.div
        ...
        animate={{ opacity: [0.3, 0.7, 0.3], scaleY: [0.8, 1, 0.8] }} /* Pulsiert in Helligkeit + Höhe */
      ...
      <motion.div
        ...
        animate={{ rotate: [0, 45, 0], opacity: [0.18, 0.35, 0.18] }} /* Dreht sich 45° hin und zurück */
...
function CustomCursor() {
  ...
  const [visible, setVisible] = useState(false); /* false bis erste Mausbewegung */
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("mousemove", onMove);     /* Cleanup: kein Speicherleck */
      document.removeEventListener("mouseleave", onLeave);
      document.removeEventListener("mouseenter", onEnter);
    };
  }, [mouseX, mouseY, visible]); /* visible in Deps damit setVisible korrekt referenziert */
    ...
    <>
      ...
      <motion.div
        style={{
          ...
          mixBlendMode: "difference", /* Invertiert Pixel darunter → Kreis ist immer sichtbar */
        }}
...
function PageLoader({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const p = Math.min((now - start) / duration, 1); /* Fortschritt 0–1, nie > 1 */
  ...
  }, [onDone]); /* onDone als Dep — ändert sich nicht, aber ESLint erwartet es */
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}        /* AnimatePresence führt diesen exit aus wenn !loaded */
      transition={{ duration: 0.6 }} /* 0.6s Fade-out */
    >
      {/* "ST" Monogramm — blendet mit scale+blur ein */}
      <motion.div
        initial={{ scale: 0.7, filter: "blur(12px)", opacity: 0 }} /* Startzustand: klein, unscharf */
        animate={{ scale: 1, filter: "blur(0px)", opacity: 1 }}    /* Endzustand: normal */
      ...
      {/* Fortschrittsbalken */}
      <div className="w-32 h-0.5 bg-white/10 rounded-full overflow-hidden"> {/* Grauer Track */}
        <motion.div
          className="h-full bg-gradient-to-r from-pink-500 to-purple-500 rounded-full"
          animate={{ width: `${progress}%` }} /* Breite folgt progress-State */
          transition={{ duration: 0.05 }}     /* Fast sofort — Balken folgt rAF-Timer */
        />
      </div>
    </motion.div>
  );
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function Navbar() {
  const [scrolled, setScrolled] = useState(false);    /* true wenn >40px gescrollt */
  const [mobileOpen, setMobileOpen] = useState(false); /* Hamburger-Menü offen? */
  /* Welcher Link gerade aktiv (hover) für die Pill-Animation */
  const [activeLink, setActiveLink] = useState<string | null>(null);

  useEffect(() => {
    const fn = () => setScrolled(window.scrollY > 40); /* 40px Schwellenwert für Glaseffekt */
    window.addEventListener("scroll", fn, { passive: true }); /* passive: scrollt nicht blockieren */
    return () => window.removeEventListener("scroll", fn);    /* Cleanup */
  }, []); /* Nur einmal registrieren */

  /* Nav-Links: Label → Anchor-ID */
  const links = [
    { label: "Funktionen", href: "#funktionen" }, /* Scrollt zu FeaturesSection */
  ...
  return (
    ...
    <motion.nav
      ...
      transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }} /* Expo-Out Kurve */
    ...
    >
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2.5 group">
          <motion.div
            whileHover={{ scale: 1.12, rotate: -3 }} /* Mini-Animation bei Hover */
            className="w-8 h-8 rounded-xl bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center shadow-lg"
          >
            <span className="text-white text-sm font-black">S</span> {/* "S" für StudyTracker */}
          </motion.div>
        ...
        <LayoutGroup> {/* LayoutGroup: erlaubt layoutId-Pill zwischen verschiedenen Links zu springen */}
          <div
            className="hidden md:flex items-center gap-1" /* hidden on mobile, flex on md+ */
            onMouseLeave={() => setActiveLink(null)} /* Pill verschwindet wenn Maus den Container verlässt */
          >
            {links.map(({ label, href }) => (
              <a
                ...
                onMouseEnter={() => setActiveLink(href)} /* Aktiven Link für Pill merken */
              >
                ...
                {activeLink === href && (
                  <motion.span
                    ...
                    transition={{ type: "spring", stiffness: 380, damping: 30 }} /* Federnd */
                  />
                )}
                <span className="relative">{label}</span> {/* relative: über der Pill */}
              </a>
            ))}
          </div>
        </LayoutGroup>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setMobileOpen((v) => !v)} /* Toggle: offen ↔ geschlossen */
          ...
          >
            <div className="w-5 flex flex-col gap-[5px]"> {/* 3 Streifen mit 5px Abstand */}
          ...
          >
            <div className="px-5 py-3 space-y-1">
              ...
              <div className="pt-2 border-t border-white/8">
                <Link
                  to="/login"
                  onClick={() => setMobileOpen(false)} /* Schließt auch beim Anmelden-Klick */
...
function Typewriter() {
  ...
  const [deleting, setDeleting] = useState(false); /* true = Löschen, false = Tippen */
  ...
  useEffect(() => {
    ...
    if (!deleting && displayed.length < target.length) {
      ...
      timeout = setTimeout(() => {
        setDeleting(false);
        setRoleIdx((i) => (i + 1) % TYPEWRITER_ROLES.length); /* Modulo: nach letzter wieder erste */
      }, 0);
    }
    return () => clearTimeout(timeout); /* Cleanup: verhindert mehrere parallele Timeouts */
  }, [displayed, deleting, roleIdx]); /* roles ist Konstante außerhalb — kein Dep nötig */
...
function MagneticButton({
  ...
  const ref = useRef<HTMLDivElement>(null); /* Referenz auf den Container für BoundingRect */
  const x = useMotionValue(0);             /* Rohe x-Verschiebung (0 = zentriert) */
  const y = useMotionValue(0);             /* Rohe y-Verschiebung */
  const springX = useSpring(x, { stiffness: 280, damping: 20 }); /* Federnde x-Verschiebung */
  const springY = useSpring(y, { stiffness: 280, damping: 20 }); /* Federnde y-Verschiebung */

  const handleMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (!ref.current) return;
      const rect = ref.current.getBoundingClientRect(); /* Position des Elements im Viewport */
  ...
  return (
    ...
    <div className="bg-white/[0.06] backdrop-blur-2xl rounded-3xl p-5 w-full max-w-xs mx-auto border border-white/12 shadow-2xl">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/15 rounded-full border border-emerald-500/25">
          <span className="relative flex h-1.5 w-1.5">
            ...
            />
            <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-400" /> {/* Innerer Kern */}
          </span>
      ...
      <div className="bg-gradient-to-br from-pink-500/15 to-purple-500/15 rounded-2xl p-4 mb-3 border border-white/8">
        <p className="text-2xl font-black text-white">12h 34m</p> {/* Beispielwert */}
          ...
          <div className="h-1 bg-white/10 rounded-full overflow-hidden">
            <motion.div
              ...
              transition={{
                ...
                ease: [0.34, 1.56, 0.64, 1], /* Leichter Überschwinger (spring-like) */
              }}
            />
          </div>
        </div>
      ...
      <div className="space-y-2 mb-3">
        {[
          { s: "Mathematik", t: "4h 20m", c: "bg-pink-500",   p: "65%" }, /* Fach, Zeit, Farbe, Prozent */
          { s: "Englisch",   t: "3h 10m", c: "bg-purple-500", p: "48%" },
          { s: "Physik",     t: "2h 05m", c: "bg-fuchsia-500",p: "31%" },
        ].map(({ s, t, c, p }) => (
          <div key={s} className="flex items-center gap-2.5">
            <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${c}`} /> {/* Farbpunkt */}
            <div className="flex-1 min-w-0"> {/* min-w-0: kein Overflow in Flex-Container */}
              <div className="flex justify-between mb-0.5">
                <span className="text-[10px] text-white/50">{s}</span>  {/* Fachname */}
                <span className="text-[10px] font-bold text-white/80">{t}</span> {/* Lernzeit */}
              </div>
              <div className="h-0.5 bg-white/8 rounded-full overflow-hidden"> {/* Track */}
                <div
...
function HeroSection() {
  ...
  const { scrollYProgress } = useScroll({
    ...
    offset: ["start start", "end start"],     /* 0 = oben sichtbar, 1 = Element-Ende oben */
  });
  ...
  return (
    ...
    >
      <motion.div
        style={{ opacity: contentOpacity }} /* Blendet aus wenn man runterscrollt */
      ...
      >
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="min-w-0 overflow-hidden">
            ...
            <motion.div
              initial={{ opacity: 0, y: 20 }} /* Startet unsichtbar, 20px tiefer */
            ...
            <div className="mb-2 overflow-hidden">
              <motion.h1
                ...
                initial={{ y: "105%" }} /* 105% statt 100% wegen Descender-Platz */
                animate={{ y: "0%" }}
                transition={{
                  duration: 0.9,
                  ease: [0.76, 0, 0.24, 1], /* Cubic-Bezier: starke Expo-Out-Kurve */
                  delay: 0.3,
                }}
              >
                STUDY
            ...
            <div className="mb-6 overflow-hidden">
              <motion.h1
                ...
                style={{
                  WebkitTextStroke: "2px rgba(168,85,247,0.7)", /* 2px lila Kontur */
            ...
            {/* Typewriter Untertitel */}
            <motion.p
              initial={{ opacity: 0 }} /* Erst einblenden nachdem Titel fertig ist */
              animate={{ opacity: 1 }}
              transition={{ delay: 0.8 }}
              className="text-base sm:text-lg text-white/60 mb-8 max-w-md"
            >
              Werde ein besserer <Typewriter /> {/* Typewriter-Komponente tippt Rollen ein */}
            </motion.p>
          ...
          <motion.div
            ...
            initial={{ opacity: 0, y: 40, scale: 0.96 }} /* Startet leicht unten, klein */
...
function ArcRing({
  ...
  const offset = circ - (percent / 100) * circ; /* Lücke im Arc (0 = komplett, circ = leer) */
...
function StatsSection() {
  ...
  const users  = useCounter(2000,  inView, 28); /* Zählt auf 2.000 in ~1.4s (28ms × 50) */
  ...
  const stats = [
    ...
    { value: `${streak} Tage`, label: "Ø Streak", arc: 48, color: "#818cf8" }, /* Indigo */
  ];
      ...
      <div className="max-w-4xl mx-auto px-5 grid grid-cols-3 gap-4 sm:gap-8">
        {stats.map(({ value, label, arc, color }, i) => (
          <motion.div
            ...
            animate={inView ? { opacity: 1, y: 0 } : {}} /* Nur animieren wenn sichtbar */
            transition={{ delay: i * 0.14, duration: 0.5 }} /* Stagger: 0, 0.14, 0.28s */
...
function MarqueeStrip() {
  ...
  const words = [
    "Fokus-Timer", "Statistiken", "Lern-Streaks", "Dark Mode",
    "Wochenziele", "Achievements", "Kategorien", "Export",
    "Auto-Speicherung", "Erinnerungen",
  ];
  const track = [...words, ...words, ...words]; /* 3× = 30 Einträge für seamless loop */
  ...
  return (
    <div className="relative py-8 overflow-hidden border-b border-white/6">
      ...
      <motion.div
        ...
        animate={{ x: ["0%", "-33.333%"] }} /* Schiebt um ein Drittel nach links */
      ...
      >
        {track.map((w, i) => (
          ...
          >
            {w} <span className="text-pink-500/40">✦</span> {/* Trenn-Diamant */}
          </span>
        ))}
      </motion.div>
    </div>
...
function FeatureCard({
  ...
  const ref = useRef<HTMLDivElement>(null); /* Ref für Viewport-Check + BoundingRect */
  ...
  const handleMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!ref.current) return;
    const r = ref.current.getBoundingClientRect(); /* Kartenposition im Viewport */
  ...
  return (
    <motion.div
      ref={ref}
      initial={{ opacity: 0, y: 32, scale: 0.96 }} /* Startet etwas tiefer und kleiner */
      animate={inView ? { opacity: 1, y: 0, scale: 1 } : {}}
      transition={{ duration: 0.55, delay, ease: [0.22, 1, 0.36, 1] }}
      whileHover={{ y: -5 }}  /* Hebt sich 5px beim Hover an */
      onMouseMove={handleMove}
      onMouseLeave={() => setSpot((s) => ({ ...s, show: false }))} /* Spotlight ausblenden */
    ...
    >
      ...
      {spot.show && (
        <div
          ...
          style={{
            ...
            width: 280,
            height: 280,
            transform: "translate(-50%,-50%)", /* Gradient-Zentrum auf Mauszeiger */
      ...
      <div className="flex flex-wrap gap-1.5">
        {tags.map((tag) => (
          <motion.span
            key={tag}
            whileHover={{ scale: 1.08, y: -2 }} /* Springt leicht hoch bei Hover */
    ...
    },
    {
      ...
      icon: (
        ...
        >
          ...
          <path d="M10 3V1M10 19v-2M3 10H1M19 10h-2" /> {/* Kreuz-Achsen (N/S/W/O) */}
        </svg>
      ),
      ...
      <div className="max-w-6xl mx-auto px-5">
        ...
        <div ref={headerRef} className="text-center mb-14">
          ...
          <div className="overflow-hidden">
            <motion.h2
              initial={{ y: "100%" }} /* Startet unterhalb des overflow-hidden Containers */
...
function AllInOneSection() {
  const leftRef  = useRef<HTMLDivElement>(null); /* Scroll-Trigger linke Spalte */
  const rightRef = useRef<HTMLDivElement>(null); /* Scroll-Trigger rechte Spalte */
  ...
  const bars = [
    ...
    { day: "Fr", h: 100, hot: true  }, /* Freitag ist der "beste" Tag — Gradient-Highlight */
            ...
            <div className="space-y-3">
              {items.map(({ emoji, title, desc }, i) => (
                <motion.div
                  ...
                  transition={{ delay: 0.2 + i * 0.1, duration: 0.4 }} /* Stagger 0.1s */
                ...
                >
                  <span className="text-xl flex-shrink-0">{emoji}</span> {/* flex-shrink-0: Emoji schrumpft nicht */}
                  <div>
            ...
            <div className="bg-white/[0.04] backdrop-blur-xl rounded-2xl p-6 border border-white/10">
              ...
              <div className="mb-3">
                <div className="relative h-28 flex gap-1.5">
                  {bars.map((bar, i) => (
                    <div key={bar.day} className="flex-1 relative"> {/* flex-1: alle Balken gleich breit */}
                      <motion.div
                        ...
                        animate={rightIn ? { scaleY: 1 } : { scaleY: 0 }}    /* Wächst wenn sichtbar */
                        transition={{
                          delay: i * 0.08 + 0.2,                              /* Stagger: jeder Balken 80ms nach dem vorherigen */
                          duration: 0.6,
                          ease: [0.34, 1.56, 0.64, 1],                        /* Leichter Überschwinger oben */
                        }}
                      />
                    </div>
                  ))}
                    ...
                    <div key={bar.day} className="flex-1 flex justify-center">
                      <span
                        className={`text-[9px] font-semibold ${
                          bar.hot ? "text-white" : "text-white/35" /* Highlight-Tag heller */
                        }`}
                      >
                        {bar.day}
                      </span>
              ...
              <div className="grid grid-cols-3 gap-2 mt-5">
                {[
                  ...
                  >
                    <p className="text-white font-black text-sm">{v}</p>   {/* Wert */}
                    <p className="text-white/35 text-[10px] mt-0.5">{l}</p> {/* Label */}
                  </div>
                ))}
              </div>
            </div>
...
function FAQSection() {
  ...
  const [openIdx, setOpenIdx] = useState<number | null>(0); /* Erste Frage standardmäßig offen */
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <section id="faq" className="relative py-20 sm:py-28">
      <div className="max-w-2xl mx-auto px-5"> {/* max-w-2xl: schmale Spalte für lesbare FAQs */}
        {/* Header */}
          ...
          {faqs.map(({ q, a }, i) => (
            ...
            >
              ...
              <button
                ...
                aria-expanded={openIdx === i} /* Accessibility: Screen-Reader Status */
              ...
              >
                ...
                >
                  ...
                  >
                    <path d="M6 2v8M2 6h8" strokeLinecap="round" /> {/* + Kreuz */}
                  </svg>
                </motion.span>
              </button>
              {/* Antwort: AnimatePresence für height 0 → auto Übergang */}
              <AnimatePresence initial={false}> {/* initial: false = kein Animate beim ersten Render */}
                {openIdx === i && (
                  <motion.div
                    key="answer"
                    initial={{ height: 0, opacity: 0 }}     /* Eingeklappt */
    ...
    <footer className="relative border-t border-white/6 py-8">
      <div className="max-w-6xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <motion.div
            whileHover={{ scale: 1.1, rotate: -3 }} /* Mini-Animation wie Navbar-Logo */
...
export default function Landing() {
  const [loaded, setLoaded] = useState(false); /* false = Loader sichtbar, true = Seite sichtbar */
